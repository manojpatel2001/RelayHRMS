    ...
    public class LevelWiseCardMappingAPIController : ControllerBase
    {
        ...
        public async Task<APIResponse> CreateOrUpdateLevelWiseCardMapping(LevelWiseCardMapping model)
        {
            try
            {
                ...
                {
                    var isExists = await _unitOfWork.LevelWiseCardMappingRepository.GetAllAsync(asd => asd.CardType.ToLower()==model.CardType.ToLower() && asd.Level.ToLower().Trim() == model.Level.ToLower().Trim() 
                    && asd.IsEnabled == true && asd.IsDeleted == false);
                    if (isExists.Any())
                    {
                        return new APIResponse() { isSuccess = false, ResponseMessage = $"Record with name '{model.CardType}' already exists" };
                    }
                ...
                else
                {
                    ...
                    var isExists = await _unitOfWork.LevelWiseCardMappingRepository.GetAllAsync(asd => asd.CardType.ToLower() == model.CardType.ToLower() && asd.Level.ToLower().Trim() == model.Level.ToLower().Trim()
                   &&asd.LevelWiseCardMappingId!=model.LevelWiseCardMappingId && asd.IsEnabled == true && asd.IsDeleted == false);
                    if (isExists.Any())
                    {
                        return new APIResponse() { isSuccess = false, ResponseMessage = $"Record with name '{model.CardType}' already exists" };
                    }
